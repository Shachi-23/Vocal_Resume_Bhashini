...
#everything onwards is added onwards 13/08/2025
@app.route('/api/process-freeform-audio', methods=['POST'])
def process_freeform_audio():
    '''Process free-form audio recording - transcribe, translate, and generate resume'''
    ...
    try:
        # Step 1: Transcribe audio using Bhashini or Google fallback
        if BHASHINI_USER_ID and BHASHINI_API_KEY:
            print("Attempting transcription with Bhashini...")
            converted_audio_path = convert_audio_for_bhashini(audio_path)
            if converted_audio_path:
        ...
        # Step 2: Translate to English if needed (using Bhashini or Google Translate)
        english_transcript = transcript
        if language != 'en':
            english_transcript = translate_to_english(transcript, language)
            if not english_transcript:
                english_transcript = transcript  # Fallback to original
        
        print(f"English transcript: {english_transcript}")
        
        # Step 3: Extract resume information using OpenRouter
        resume_data = extract_resume_from_freeform_text(english_transcript, profession)
        
        if not resume_data:
            return jsonify({'message': 'Failed to extract resume information'}), 500
        
        # Step 4: Generate resume in both languages
        resume_data['originalLanguage'] = language
        resume_data['originalTranscript'] = transcript
        resume_data['englishTranscript'] = english_transcript
        
        # Translate resume back to original language if needed
        if language != 'en':
            resume_data['translatedResume'] = translate_resume_to_language(resume_data, language)
        
        return jsonify({
            'message': 'Free-form audio processed successfully',
            'data': resume_data,
            'transcript': transcript,
        ...
        app.logger.error(f"Error processing free-form audio: {e}")
        return jsonify({'message': f'Error processing audio: {str(e)}'}), 500
...
def translate_to_english(text, source_language):
    '''Translate text to English using available translation services'''
    # Try Bhashini first
    if BHASHINI_USER_ID and BHASHINI_API_KEY:
        try:
            translated = translate_with_bhashini(text, source_language, 'en')
            if translated:
                return translated
        except Exception as e:
    ...
    # This would need to be implemented based on available services
    return text


def extract_resume_from_freeform_text(transcript, profession):
    '''Extract structured resume information from free-form transcript using OpenRouter'''
    ...
    prompt = f'''You are a professional resume writer and career counselor. I will provide you with a free-form transcript where someone talks about their professional journey, and you need to extract and organize this information into a structured resume format.
...
Please analyze the transcript and extract the following information to create a professional resume. The person works as a {profession}.

INSTRUCTIONS:
1. Extract personal information (name, contact details) if mentioned
2. Create a professional summary based on their story
3. List work experience in reverse chronological order
4. Include education background
5. Extract skills mentioned or implied
6. Include any projects, certifications, or achievements mentioned
7. Note any languages they speak

Return the response as a valid JSON object with this exact structure:
{json.dumps(resume_structure, indent=2)}

IMPORTANT:
- If information is not mentioned in the transcript, leave those fields empty
- For experience, include company names, job titles, duration, and key responsibilities
- For education, include degree, institution, and year if mentioned
- Extract both technical and soft skills
- Create a compelling summary that highlights their journey and expertise
- Ensure all dates and durations are properly formatted
- If they mention years of experience, calculate approximate start dates

Only return valid JSON, no markdown formatting or additional text.'''

    try:
        response_text = call_openrouter_api(prompt, model="openai/gpt-4o")
        ...
        print(f"Error extracting resume from transcript: {e}")
        return None


def translate_resume_to_language(resume_data, target_language):
    '''Translate resume content back to the original language'''
    # This would implement translation of the resume content
    # back to the user's preferred language using available translation services
    return resume_data


print("Free-form audio processing endpoint code generated.")
print("This endpoint should be added to the Flask backend (user-OUYzK.py)")
print("The endpoint handles:")
print("1. Audio transcription using Bhashini/Google")
print("2. Translation to English if needed")
print("3. Resume extraction using OpenRouter")
print("4. Translation back to original language")
